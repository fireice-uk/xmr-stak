...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	ctx->InputBuffer = clCreateBuffer(opencl_ctx, CL_MEM_READ_ONLY, amd_max_input_len + 4, NULL, &ret);
	if(ret != CL_SUCCESS)
	{
...
size_t XMRSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, const xmrstak_algo& miner_algo)
{
	...
	if(input_len > amd_max_input_len)
	{
		printer::inst()->print_msg(L1,"Error: job input length too long: %ld > %ld", input_len, amd_max_input_len);
		return ERR_STUPID_PARAMS;
	}

	input[input_len] = 0x01;
	memset(input + input_len + 1, 0, amd_max_input_len + 4 - input_len - 1);

	cl_uint numThreads = ctx->rawIntensity;

	if((ret = clEnqueueWriteBuffer(ctx->CommandQueues, ctx->InputBuffer, CL_TRUE, 0, amd_max_input_len + 4, input, 0, NULL, NULL)) != CL_SUCCESS)
	{
